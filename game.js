...
function drawAnimeVNScene(level, seedBase){const c=$("sceneCanvas"),ctx=c.getContext("2d"),w=c.width,h=c.height,seed=hashSeed(level.sceneTitle+level.concept+seedBase);const sky=ctx.createLinearGradient(0,0,0,h);sky.addColorStop(0,`hsl(${Math.floor(seeded(seed)*360)},75%,20%)`);sky.addColorStop(1,`hsl(${Math.floor(seeded(seed+1)*360)},65%,10%)`);ctx.fillStyle=sky;ctx.fillRect(0,0,w,h);for(let i=0;i<28;i++){const x=seeded(seed+i*11)*w,y=seeded(seed+i*19)*(h*.7),r=40+seeded(seed+i*7)*100,g=ctx.createRadialGradient(x,y,0,x,y,r);g.addColorStop(0,`hsla(${(i*37)%360},90%,75%,0.25)`);g.addColorStop(1,"hsla(0,0%,100%,0)");ctx.fillStyle=g;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()}ctx.fillStyle="rgba(20,30,80,.45)";for(let i=0;i<8;i++){const x=i*130+20,hh=120+seeded(seed+i)*180;ctx.fillRect(x,h-hh-80,90,hh)}ctx.fillStyle="rgba(248,236,220,.98)";ctx.beginPath();ctx.arc(w*.72,h*.31,62,0,Math.PI*2);ctx.fill();ctx.fillStyle="#5a3d2f";ctx.beginPath();ctx.ellipse(w*.72,h*.27,70,55,0,0,Math.PI*2);ctx.fill();ctx.fillStyle="#f6fbff";ctx.fillRect(w*.63,h*.39,180,220);ctx.fillStyle="#7aa2ff";ctx.fillRect(w*.68,h*.46,70,28);ctx.fillStyle="rgba(220,232,255,.85)";ctx.fillRect(w*.44,h*.45,110,170);ctx.fillStyle="#89b4ff";ctx.beginPath();ctx.arc(w*.495,h*.45,50,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(12,14,35,.78)";ctx.strokeStyle="rgba(255,182,219,.85)";ctx.lineWidth=2;ctx.fillRect(24,h-170,w-48,146);ctx.strokeRect(24,h-170,w-48,146);ctx.fillStyle="#ffd7ef";ctx.font="bold 30px sans-serif";ctx.fillText(level.sceneTitle,42,h-132);ctx.fillStyle="#eaf0ff";ctx.font="24px sans-serif";ctx.fillText(`科学概念：${level.concept}`,42,h-92)}
...
$("demoBtn").onclick=()=>{$("storyInput").value=`在未来都市校园中，我和AI助手要修复失控能源塔。\nQ: 为了判断电路是否过载，最先应做什么？\nA) 直接更换所有元件\nB) 测量电流并与额定值比较\nC) 提高电压看看会不会跳闸\nANS: B\nSCI: 电路\nQ: 气压阀门打不开时，合理步骤是？\nA) 猛拉阀门\nB) 先平衡两侧压差再开启\nC) 敲击阀体\nANS: B\nSCI: 空气压力`};
